...
async checkMovieExists(movieName) {
  const response = await axios.get('sysMovie/checkExists', { params: { movieName } });
  return response.data.data; // 假设后端返回 { exists: true/false }
},
async addMovie() {
    // 验证电影名称是否已存在
    const exists = await this.checkMovieExists(this.addForm.movieName);
    console.log('电影名称是否存在:', exists);
    if (exists) {
        this.$message.error('电影名称已存在，请使用其他名称！');
        return; // 终止添加操作
    }

    // 验证电影时长是否大于0
    if (this.addForm.movieLength <= 0) {
        this.$message.error('电影时长必须大于0！');
        return; // 终止添加操作
    }
