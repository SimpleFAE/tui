  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-table :data="movieCategoryList" style="width: 45%" border stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column label="类别编号" width="145">
...
export default {
  ...
  methods: {
    ...
    this.$refs.addFormRef.validate(async valid => {
        ...
        // 检查类别名称是否已存在
        const exists = this.movieCategoryList.some(category => category.movieCategoryName === this.addForm.movieCategoryName);
        if (exists) {
            this.$message.error('类别名称已存在，请使用其他名称！');
            return; // 终止添加操作
        }
