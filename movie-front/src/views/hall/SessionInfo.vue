...
<el-form-item label="播放时间" prop="playTime">
  <el-time-picker
    ...
    :disabled="!addForm.sessionDate"
    @change="validatePlayTime">
  </el-time-picker>
</el-form-item>
...
<el-form-item label="播放时间" prop="playTime">
  <el-time-picker
    ...
    placeholder="选择播放时间"
    :picker-options="timePickerOptions"
    :disabled="!editForm.sessionDate"
    @change="validateEditPlayTime">
  </el-time-picker>
</el-form-item>
        <el-form-item label="场次价格" prop="sessionPrice">
...
export default {
  ...
  methods: {
    ...
    if (date === moment().format('YYYY-MM-DD')) {
      ...
      this.timePickerOptions.selectableRange = `${hours} - 23:59:59`;
    } else {
      this.timePickerOptions.selectableRange = '00:00:00 - 23:59:59';
    }
    this.editForm.playTime = '';
  },

  validateEditPlayTime(time) {
    // 与添加表单类似的时间验证逻辑
    if (!time) return;
    
    const now = moment();
    const selectedDateTime = moment(`${this.editForm.sessionDate} ${time}`);
  ...
  handleDateChange(date) {
    ...
    // 清空已选择的时间
    this.addForm.playTime = '';
  },

  // 验证播放时间
  validatePlayTime(time) {
    if (!time) return;
    
    const now = moment();
    const selectedDateTime = moment(`${this.addForm.sessionDate} ${time}`);
    
    // 如果选择的时间早于当前时间，显示错误提示并清空选择
    if (selectedDateTime.isBefore(now)) {
      this.$message.error('不能选择过去的时间');
      this.addForm.playTime = '';
    }
    ...
    addSession(){
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        if (!valid) return this.$message.error('添加场次信息失败')
          // 验证日期和时间
      const selectedDateTime = moment(`${this.addForm.sessionDate} ${this.addForm.playTime}`);
    ...
    async deleteSessionById(id){
      ...
      try {
        const response = await axios.delete('sysSession/' + id);
        if (response.data.code === 200) {
            this.$message.success('删除场次信息成功！');
            this.getSessionList(); // 重新获取场次信息
        } else {
