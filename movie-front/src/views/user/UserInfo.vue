...
export default {
  ...
  methods: {
    ...
    this.$refs.addFormRef.validate(async valid => {
        ...
        try {
            const resp = await axios.post('sysUser', JSON.stringify(_this.addForm));
            if (resp.data.code !== 200) {
                this.$message.error(`添加用户失败！原因: ${resp.data.msg || '未知错误'}`);
                success = false;
            }
        } catch (error) {
            // 捕获并处理错误
            this.$message.error(`添加用户失败！原因: ${error.response ? error.response.data.msg : '网络错误'}`);
            success = false;
        }
