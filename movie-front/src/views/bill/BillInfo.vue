<template>
  ...
  <template slot-scope="scope">
    {{ scope.row.sysSession ? scope.row.sysSession.playTime : '无数据' }} - {{ scope.row.sysSession ? scope.row.sysSession.endTime : '无数据' }}
  </template>
</el-table-column>
    ...
    <el-dialog title="修改订单" :visible.sync="editDialogVisible" width="60%" @close="editDialogClosed">
      <el-form :model="editForm" ref="editFormRef" label-width="100px">
        ...
        <el-form-item label="播放时间" prop="sysSession.playTime">
  <el-input v-model="editForm.sysSession.playTime" disabled></el-input>
</el-form-item>
<el-form-item label="散场时间" prop="sysSession.endTime">
...
export default {
  ...
  // 确保 sysSession 存在
  if (!_this.editForm.sysSession) {
    this.$message.error('订单信息不完整，无法修改。');
    return;
  }
